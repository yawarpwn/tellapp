...
import { usePDF, PDFDownloadLink, Page, Document } from '@react-pdf/renderer';
import PDFGenerator from './PDFGenerator';

export default function QuotationPrint() {
  const store = useQuotationStore()
  const [instance] = usePDF({ document: <PDFGenerator quotation={store.quoToEdit} /> })
    ...
    <aside onClick={event => {
      ...
      <article className="max-w-[768px] h-screen p-4 bg-white">
        ...
        <div className='grid grid-cols-2 gap-x-4'>
          <a href={instance.url} download={`COT-2023-00${store.quoToEdit.quoNumber}.pdf`} className='py-4 px-6 bg-purple-500 text-white text-center' >
